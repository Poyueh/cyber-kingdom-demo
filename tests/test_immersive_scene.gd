...
func run_scene() -> void:
 ...
 check(game.knight.visual.breathing and is_equal_approx(game.knight.velocity.x,game.tuning.walking_speed*game.sim.travel.rules.tired_speed_multiplier),"exhaustion selects the slower tired walking pace")
 key(KEY_SHIFT,false);key(KEY_D,false);await frames(52)
 ...
 key(KEY_D,true);await frames(52)
