...
const LEVELS := {"music": 0.4, "effects": 0.8, "ambience": 0.6}
var writable:=true
var path: String
...
func read() -> Dictionary:
	...
	for key in LEVELS:
		...
		var value=file.get_value("audio",key,LEVELS[key])
		if not (value is float or value is int) or not is_finite(float(value)) or value<0 or value>1:
			return _rejected()
		result[key]=float(value)
	var muted=file.get_value("audio","muted",false)
	if not (muted is bool):
		return _rejected()
	result.muted=muted
	return result
func write(music: float,effects: float,ambience: float,muted: bool) -> bool:
