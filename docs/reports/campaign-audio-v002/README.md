...
日期 2026-09-23。本檔記錄素材的生成與驗收（計畫的 P0）。素材之後已全部接進遊戲，遊戲執行時播的就是這批 v002 素材，v001 佔位音不再使用；接線細節見 `docs/design/campaign-audio-v002.md`，實際發聲證據見本目錄的 `native-playback.json`。
...
1. 使用者與 PO 依上面的試聽檔確認音色，逐條回報要改的項目。
2. 要改的就改 `tools/audio_v002_manifest.py` 的 prompt，刪掉該檔的暫存原始檔後重跑上面五個指令，再跑 `bash tools/check.sh`。
3. 音色以外，工作敲擊與腳步的節奏手感需要實機遊玩確認，這個自動測試測不出來。
