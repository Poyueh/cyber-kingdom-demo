# 戰役音效與音樂 v002 全面重製計畫（草案，待使用者審閱）

> 狀態：P0 已完成（2026-09-23）。116 個素材已生成並通過音量與人聲檢查，尚未接進遊戲，執行時仍播 v001。
> 產出與驗收見 `docs/reports/campaign-audio-v002/README.md`，逐條 prompt 見 `docs/design/campaign-audio-v002-catalogue.md`。
...
## 7. 分階段實作（每階段一個 M 級工作單元，先寫失敗測試）

1. **P0 素材生成與試聽**：使用者以選定工具依第 4、5 節 prompt 生成，放進 v002 目錄；我跑後處理腳本、寫 provenance.json、原生錄一段混音供試聽。音色未認可前不進 P1。
2. **P1 bus 與播放器換血**：新增 bus layout；CampaignAudio 改讀 v002 素材與變體輪播；拆出 CampaignMusic 與狀態機；偏好檔加環境音量；暫停低通。測試：狀態切換、交叉淡變不重疊播放、偏好三欄讀寫與壞資料拒絕。
3. **P2 觀察器補事件**：第 6 節「新增／改對應／拆開」全部接上。測試：每個新事件各一條「發生才播、重播不補」；掉晶與掉劍依 crystal-survival 規則在觸發端測。
4. **P3 環境音**：CampaignAmbience 依鏡頭 x 與日夜混合五個循環，定位型（營火、晶脈、遺跡）依距離衰減。測試：位置與時間對應的目標增益。
5. **P4 UI 與靜音偏好**：UI 音接 HUD 與起始頁；靜音狀態寫入偏好檔跨啟動。測試：靜音持久化、UI 音不受 SFX 限流影響。
6. **P5 居民工作音**：工匠／農夫／獵人動作各自定位發聲，遠處靜音，同幀合併。測試：範圍外不發聲、多人同時工作不超過槽數。
7. 每階段：`bash tools/check.sh` 零 SCRIPT ERROR、更新 `docs/STATUS.md`、更新 `campaign-audio.md` 為 v002 規格、第 83 課教學（Audio Bus Layout 與 AudioStreamPlayer 淡變）。
